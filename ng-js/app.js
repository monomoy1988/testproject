...
jungledrone.run(['$rootScope', '$state','$cookieStore','$interval','$uibModal','$timeout','$compile','carttotal',function($rootScope, $state,$cookieStore,$interval,$uibModal,$timeout,$compile, carttotal){
    ...
    $rootScope.$on('$stateChangeStart',function(event, toState, toParams, fromState, fromParams){
     ...
     //   console.log($rootScope.userid+'state change user id');
        ...
        $rootScope.carttotal=parseInt(carttotal.getcontent('http://admin.westcoastvg.online/carttotal?user='+$rootScope.cartuser));
        ...
        $cookieStore.put('currenttime',Math.floor(Date.now()));
        var prevtime=$cookieStore.get('currenttime');

        if(typeof ($cookieStore.get('lastactive')) != 'undefined'){
            var lastactive=$cookieStore.get('lastactive');
            var diffnew = prevtime-lastactive;
            diffnew = diffnew/1000;
            diffnew = parseInt(diffnew);
        ...
        if(toState.name != 'index' && toState.name != 'login' && toState.name != 'login2nd' && toState.name != 'userreset' && toState.name != 'login2' && toState.name != 'forgot-password' && toState.name != 'resetpassword' && toState.name != 'forgot-password-check' && toState.name != 'change-password'){
            $rootScope.promise = $interval(function(){
                var curtime = Math.floor(Date.now());
                var diff2 = curtime-prevtime;
                diff2 = diff2/1000;
                ...
                if(diff2 == 600){
                    ...
                    $uibModal.open({
                        animation: true,
                        template : '<div class="confirm_delpopup" >\
                        <a ng-click="autologout()" class="shutter-close2 ng-scope" href="javascript:void(0)"><img src="images/newloginclose.png"> </a>\
                        <div class="clearfix"></div>\
                        <h2 style="text-align: left; font-size: 20px; padding: 10px 0;">You will automatically logged out in 1 min.  Please click here to stay logged in!!!</h2>\
                    <div class="confirm_delbtn ng-scope">\
                        <input type="button" class="subbtn" value="Ok" ng-click="staylogin()">\
                        <input type="button" class="subbtn" value="Cancel" ng-click="autologout()">\
                        <div class="clearfix"></div>\
                        </div>\
\
                        <div class="clearfix"></div>\
                        </div>',
                        controller: 'ModalInstanceCtrl55',
                        size: 'md',
                        scope:$rootScope
                    });
    ...
    $rootScope.$on('$stateChangeSuccess',function(ev, to, toParams, from, fromParams){
        ...
        setInterval(function () {
            $cookieStore.put('lastactive',Math.floor(Date.now()));
        },6000);
...
//
    ...
    $stateProvider
        ...
        .state('userreset',{
                url:"/user/reset/:rstr/:uid",
                views: {
                    'footer': {
                        templateUrl: 'partials/footer.html' ,
                    },
                    'content': {
                        templateUrl: 'partials/userreset.html' ,
       ...
       /* .state('employment',{
        ...
        .state('documenttrashlist',{
                ...
                views: {
                    ...
                    'content': {
                        templateUrl: 'partials/documenttrashlist.html' ,
                        controller: 'documenttrashlist'
                    },
        ...
        .state('legendlist',{
            ...
            views: {
                ...
                'content': {
                    templateUrl: 'partials/legendlist.html' ,
                    controller: 'legendlist'
                },
        ...
        .state('addlegend',{
                ...
                views: {
                    ...
                    'content':{
                        templateUrl:'partials/addlegend.html',
                        controller:'addlegend'
                    },
        ...
        .state('editlegend',{
                ...
                views: {
                    ...
                    'content':{
                        templateUrl:'partials/editlegend.html',
                        controller:'editlegend'
                    },
...
jungledrone.controller('resetpassword', function($scope,$state,$http,$cookieStore,$rootScope,$stateParams,$timeout) {
    ...
    $scope.custompassword=function(){
        if(typeof ($scope.form.newpassword)=='undefined') return "Password must contain at least one lower case character. Password must contain at least one upper case character. Password must contain at least one integer .Password must contain at least one special charecter and must be more than 8 charecters such as '^!,@,#,$,%,^,&,*,(,),_,+,' !!" ;
        var error=0;
        if($scope.form.newpassword.length<8) error=1;
        if($scope.form.newpassword.replace(/[^A-Z]/g, "").length<1) error=1;
        if($scope.form.newpassword.replace(/[^a-z]/g, "").length<1) error=1;
        if($scope.form.newpassword.replace(/[^0-9]/g, "").length<1) error=1;
        if($scope.form.newpassword.replace(/[^!@#$%^&*()_+]/g, "").length<1) error=1;

        if(error==1) return "Password must contain at least one lower case character. Password must contain at least one upper case character. Password must contain at least one integer .Password must contain at least one special charecter and must be more than 8 charecters such as '^!,@,#,$,%,^,&,*,(,),_,+,' !!" ;
    ...
    $scope.submit = function(){
        ...
        $http({
            method  : 'POST',
            async:   false,
            url     : $scope.adminUrl+'custuser/resetpass',
...
jungledrone.controller('ModalInstanceCtrl55', function($scope,$state,$cookieStore,$http,$uibModalInstance,$rootScope,Upload,$uibModal,$timeout,$interval) {

    $timeout.cancel($rootScope.timepromise);

    $rootScope.timepromise = $timeout(function(){
...
jungledrone.controller('ModalInstanceCtrl', function($scope,$state,$cookieStore,$http,$uibModalInstance,$rootScope,Upload,$uibModal,$timeout,$interval) {
    ...
    $scope.jungleconfirmdocdelete=function(){
        ...
        $http({
            method  : 'POST',
            async:   false,
            url     : $scope.adminUrl+'document/gototrash',
    ...
    $scope.docresotreconf=function(){
        ...
        $http({
            method  : 'POST',
            async:   false,
            url     : $scope.adminUrl+'document/restore',
            data    : $.param({id: $scope.documentlist[idx].id}),  // pass in data as strings
            headers : { 'Content-Type': 'application/x-www-form-urlencoded' }
        }) .success(function(data) {
            $rootScope.stateIsLoading = false;
    ...
    $scope.legendconfdelete = function () {
        $uibModalInstance.dismiss('cancel');
        $rootScope.stateIsLoading = true;
        var idx = $scope.currentindex;
        $http({
            method  : 'POST',
            async:   false,
            url     : $scope.adminUrl+'legends/delete',
            data    : $.param({id: $scope.legendlist[idx].id}),  // pass in data as strings
            headers : { 'Content-Type': 'application/x-www-form-urlencoded' }
        }) .success(function(data) {
            $rootScope.stateIsLoading = false;

            $scope.legendlist.splice(idx,1);
        });
    }

    $scope.legendconfcngstatus=function(){
        $uibModalInstance.dismiss('cancel');
        $rootScope.stateIsLoading = true;
        var idx = $scope.currentindex;
        $http({
            method  : 'POST',
            async:   false,
            url     : $scope.adminUrl+'legends/updatestatus',
            data    : $.param({id: $scope.legendlist[idx].id,status:$scope.legendlist[idx].status}),  // pass in data as strings
            headers : { 'Content-Type': 'application/x-www-form-urlencoded' }
        }) .success(function(data) {
            $rootScope.stateIsLoading = false;

            if($scope.legendlist[idx].status == 0){
                $scope.legendlist[idx].status = 1;
            }else{
                $scope.legendlist[idx].status = 0;
            }
        });
    }
...
jungledrone.controller('index', function($scope,$state,$http,$cookieStore,$rootScope,$stateParams,contentservice) {
    if (typeof($cookieStore.get('userid')) != 'undefined' && $cookieStore.get('userid') > 0) {
        if (typeof($cookieStore.get('isadmin')) != 'undefined' && $cookieStore.get('isadmin') == 1) {
            $state.go('dashboard');
        }else {
            $state.go('mydownloads', {id: 0});
        }
...
*/
    ...
    //if($rootScope.userrole==7 || $rootScope.userrole==4){
        ...
        $http({
            ...
            $rootScope.filenotdownloaded=new Array();
            var arr=new Array();
            //var arr={};
            $rootScope.filedownloaded=data.downloaded;
            //$rootScope.notdownloadcount=data.notdownloadcount;

            console.log($cookieStore.get('userroles'));
            console.log('Not downloaded');
            console.log(data.notdownloaded);
            var x;
            for(x in data.notdownloaded){
                //var i=getMatch(data[x].share_with_array,  $cookieStore.get('userroles'));
                //alert(data.notdownloaded[x].share_with_array);
                //alert(i);
                var y;
                for(y in data.notdownloaded[x].share_with_array){
                    //if(data[x].share_with_array.id)
                    if($.inArray(data.notdownloaded[x].share_with_array[y].id, $cookieStore.get('userroles') )>-1){
                        //$rootScope.filenotdownloaded.push(data.notdownloaded[x]);
                        if($.inArray(data.notdownloaded[x].file_name, arr )==-1){
                            $rootScope.filenotdownloaded.push(data.notdownloaded[x]);
                            arr.push(data.notdownloaded[x].file_name);
                        }

                        //$rootScope.filenotdownloaded[data.notdownloaded[x]['id']] = data.notdownloaded[x];
                        break;
                    }
                }

            }
            $rootScope.notdownloadcount=$rootScope.filenotdownloaded.length;
            //alert($rootScope.notdownloadcount);
        })
    ...
    $rootScope.accountmodal1=function(){
        console.log(11);
        $('#fwebtourmodal1').modal('hide');

    }


    $rootScope.accountmodal2=function(){
        console.log(11);
        $('#fwebtourmodal2').modal('hide');

    }


    $rootScope.accountmodal3=function(){
        console.log(11);
        $('#fwebtourmodal3').modal('hide');

    }

    $rootScope.accountmodal4=function(){
        console.log(11);
        $('#fwebtourmodal4').modal('hide');

    }

    $rootScope.accountmodal5=function(){
        console.log(11);
        $('#fwebtourmodal5').modal('hide');

    }

    $rootScope.accountmodal6=function(){
        console.log(11);
        $('#fwebtourmodal6').modal('hide');

    }

    $rootScope.accountmodal7=function(){
        console.log(11);
        $('#fwebtourmodal7').modal('hide');

    }


    $rootScope.accountmodal8=function(){
        console.log(11);
        $('#fwebtourmodal8').modal('hide');

    }

    $rootScope.accountmodal9=function(){
        console.log(11);
        $('#fwebtourmodal9').modal('hide');

    }


    $rootScope.accountmodal10=function(){
        console.log(11);
        $('#fwebtourmodal10').modal('hide');

    }

    $rootScope.accountmodal11=function(){
        console.log(11);
        $('#fwebtourmodal11').modal('hide');

    }

    $rootScope.accountmodal12=function(){
        console.log(11);
        $('#fwebtourmodal12').modal('hide');

    }

    $rootScope.accountmodal13=function(){
        console.log(11);
        $('#fwebtourmodal13').modal('hide');

    }
    $rootScope.accountmodal14=function(){
        console.log(11);
        $('#fwebtourmodal14').modal('hide');

    }

    $rootScope.accountmodal15=function(){
        console.log(11);
        $('#fwebtourmodal15').modal('hide');

    }

    $rootScope.accountmodal16=function(){
        console.log(11);
        $('#fwebtourmodal16').modal('hide');
    ...
    $http({
        method:'POST',
        async:false,
        url:$scope.adminUrl+'legends/list?filter=status&order=priority',
        headers :   { 'Content-Type': 'application/x-www-form-urlencoded' }
    }).success(function(data){
        $scope.legendlist=data;
    })
...
jungledrone.controller('login2', function($scope,$state,$http,$cookieStore,$rootScope) {
    ...
    $scope.resentmail = function () {
        ...
        $http({
            method  : 'POST',
            async:   false,
            url     : $scope.adminUrl+'custuser/resentcode',
            data    : $.param({uid:$cookieStore.get('loginuserid')}),  // pass in data as strings
            headers : { 'Content-Type': 'application/x-www-form-urlencoded' }
        }) .success(function(data) {
            $rootScope.stateIsLoading = false;
            console.log(data);
...
jungledrone.controller('userreset', function($scope,$state,$http,$cookieStore,$rootScope,$stateParams,$timeout) {
    $scope.rstr=$stateParams.rstr;
    $scope.uid=$stateParams.uid;
    $scope.isLoad = 0;
    ...
    $http({
        method  : 'POST',
        async:   false,
        url     : $scope.adminUrl+'custuser/emailverify',
        data    : $.param({'uid':$scope.uid}),  // pass in data as strings
        headers : { 'Content-Type': 'application/x-www-form-urlencoded' }
    }) .success(function(data) {
        $timeout(function(){
            $rootScope.stateIsLoading = false;
...
jungledrone.controller('addadmin', function($scope,$state,$http,$cookieStore,$rootScope) {
    ...
    $scope.submitadminForm = function(){
        ...
        $http({
            method  : 'POST',
            async:   false,
            url     : $scope.adminUrl+'custuser/addadmin',
            data    : $.param($scope.form),  // pass in data as strings
            headers : { 'Content-Type': 'application/x-www-form-urlencoded' }
        }) .success(function(data) {
            //$rootScope.stateIsLoading = false;
    ...
    $scope.custompassword=function(){
        ...
        if(typeof ($scope.form.password)=='undefined') return "Password must contain at least one lower case character. Password must contain at least one upper case character. Password must contain at least one integer .Password must contain at least one special charecter and must be more than 8 charecters such as '^!,@,#,$,%,^,&,*,(,),_,+,' !!" ;
        var error=0;
        if($scope.form.password.length<8) error=1;
        if($scope.form.password.replace(/[^A-Z]/g, "").length<1) error=1;
        if($scope.form.password.replace(/[^a-z]/g, "").length<1) error=1;
        if($scope.form.password.replace(/[^0-9]/g, "").length<1) error=1;
        if($scope.form.password.replace(/[^!@#$%^&*()_+]/g, "").length<1) error=1;
...
jungledrone.controller('editadmin', function($scope,$state,$http,$cookieStore,$rootScope,$stateParams){
    ...
    $scope.update = function () {
        ...
        $http({
            method  : 'POST',
            async:   false,
            url     : $scope.adminUrl+'custuser/adminupdate',
            data    : $.param($scope.form),  // pass in data as strings
            headers : { 'Content-Type': 'application/x-www-form-urlencoded' }
        }) .success(function(data) {
            $rootScope.stateIsLoading = false;
...
jungledrone.controller('adminlist', function($scope,$state,$http,$cookieStore,$rootScope) {
    ...
    $http({
        method  : 'POST',
        async:   false,
        url     : $scope.adminUrl+'custuser/adminlist',
        // data    : $.param($scope.form),  // pass in data as strings
        headers : { 'Content-Type': 'application/x-www-form-urlencoded' }
    }) .success(function(data) {
        $rootScope.stateIsLoading = false;
...
jungledrone.controller('stockdetail', function($scope,$state,$http,$cookieStore,$rootScope,$stateParams,$uibModal) {
    ...
    $scope.sizechoice=function(val,$ev1,type){
        if ($rootScope.userid > 0) {
            ...
            $scope.sizechoiceval = false;
            $scope.sizeid = val;
            if (type == 1)$('.tabdownloadbtn').attr('href', 'http://admin.westcoastvg.online/filedownload/id/' + $scope.id + '/' + $scope.sizeid);
            if (type == 2)$('.tabdownloadbtn').attr('href', 'http://admin.westcoastvg.online/filedownloadvideo/id/' + $scope.id + '/' + $scope.sizeid);
...
jungledrone.controller('dashboard', function($scope,$state,$http,$cookieStore,$rootScope) {
    ...
    $scope.hidemodal1=function(){
       ...
       console.log(111);
        $('#webtourmodal1').modal('hide');
        $('#webtourmodal12').modal('hide');
    }

    $scope.hidemodal2=function(){

        console.log(111);
        $('#webtourmodal2').modal('hide');
    ...
    $scope.hidemodal4=function(){
        ...
        console.log(111);
        $('#webtourmodal4').modal('hide');

    }
    $scope.hidemodal5=function(){

        console.log(111);
        $('#webtourmodal5').modal('hide');

    }
    $scope.hidemodal6=function(){

        console.log(111);
        $('#webtourmodal6').modal('hide');

    }
    $scope.hidemodal7=function(){

        console.log(111);
        $('#webtourmodal7').modal('hide');

    }
    $scope.hidemodal8=function(){

        console.log(111);
        $('#webtourmodal8').modal('hide');

    }
    $scope.hidemodal9=function(){

        console.log(111);
        $('#webtourmodal9').modal('hide');

    }
    $scope.hidemodal10=function(){

        console.log(111);
        $('#webtourmodal10').modal('hide');

    }
    $scope.hidemodal11=function(){

        console.log(111);
        $('#webtourmodal11').modal('hide');

    }
    $scope.hidemodal12=function(){

        console.log(111);
        $('#webtourmodal12').modal('hide');

    }
    $scope.hidemodal13=function(){

        console.log(111);
        $('#webtourmodal13').modal('hide');

    }
    $scope.hidemodal14=function(){

        console.log(111);
        $('#webtourmodal14').modal('hide');
    ...
    $scope.hidemodal15=function(){

        console.log(111);
        $('#webtourmodal15').modal('hide');

    }


    $scope.hidemodal16=function(){

        console.log(111);
        $('#webtourmodal16').modal('hide');

    }

    $scope.hidemodal17=function(){

        console.log(111);
        $('#webtourmodal17').modal('hide');

    }

    $scope.hidemodal18=function(){

        console.log(111);
        $('#webtourmodal18').modal('hide');

    }

    $scope.hidemodal19=function(){

        console.log(111);
        $('#webtourmodal19').modal('hide');

    }

    $scope.hidemodal20=function(){

        console.log(111);
        $('#webtourmodal20').modal('hide');

    }

    $scope.hidemodal21=function(){

        console.log(111);
        $('#webtourmodal21').modal('hide');

    }

    $scope.hidemodal22=function(){

        console.log(111);
        $('#webtourmodal22').modal('hide');
...
jungledrone.controller('documentcategorylist',function($scope,$state,$http,$cookieStore,$rootScope,$uibModal,$sce,$filter){
    ...
    $http({
        ...
        $scope.categorylist=data;
        var x;
        /*for(x in $scope.categorylist){

            alert(JSON.stringify($scope.categorylist[x]));
            //break;
        }*/
    ...
    $scope.jungledelcategory = function(item,size){
        ...
        var idx = $scope.categorylist.indexOf(item);
        $http({
            method  : 'POST',
            async:   false,
            url     : $scope.adminUrl+'document/deletecategory',
...
jungledrone.controller('adddocument',function($scope,$state,$http,$cookieStore,$rootScope,Upload,$sce,$uibModal){
    ...
    $rootScope.categorytree=function(id,type,file_url){
        console.log(9900);
        $rootScope.catlisttree=$scope.categorylist;
        $uibModal.open({
            animation: true,
        ...
        setTimeout(function () {
            ...
            //alert(4);
            var x;
            for(x in $rootScope.catlisttree){




                console.log($rootScope.catlisttree[x]);
                if($rootScope.catlisttree[x].parent_cat!=0){
                    if($( '#cattree'+$rootScope.catlisttree[x].parent_cat).parent().find('ul').length==0) $( '#cattree'+$rootScope.catlisttree[x].parent_cat).parent().append("<ul style='display:none' ></ul>");
                    var cattreeid='cattree'+$rootScope.catlisttree[x].id;
                    $( '#cattree'+$rootScope.catlisttree[x].parent_cat ).parent().find('ul').eq(0).append( "<li  id="+$rootScope.catlisttree[x].cat_name+"><span class='ctree' id="+cattreeid+">"+$rootScope.catlisttree[x].cat_name+"</span></li>" );

                }

                if($('#cattree'+$rootScope.catlisttree[x].parent_cat).length==1){
                    var expandid='expand'+$rootScope.catlisttree[x].parent_cat;
                    if($( '#cattree'+$rootScope.catlisttree[x].parent_cat).find('span.expand').length==0)$( '#cattree'+$rootScope.catlisttree[x].parent_cat).append( "<span style='cursor: pointer' class='expand' id="+expandid+"><span class='glyphicon glyphicon-chevron-down'></span> </span>" );
                }
            }
            ...
            $('.ctree').click(function () {
                console.log('ctree count');
                console.log($('.ctree').length);
                $(this).toggleClass('ctreeselected');
                //alert($("select[name='category_id']").html());
                var treeid=$(this).attr('id').toString().replace('cattree','');
                //alert($("select[name='category_id']").val());
                if($(this).hasClass('ctreeselected'))
                    $("select[name='category_id']").find("option[value='"+treeid+"']").prop('selected',true);
                    else $("select[name='category_id']").find("option[value='"+treeid+"']").prop('selected',false);

            });

            var y;
            var selectedarr=$("select[name='category_id']").val();
            for(y in selectedarr){

                $('#cattree'+selectedarr[y]).click();
            }
        },1000);
...
jungledrone.controller('documentlist',function($scope,$state,$http,$cookieStore,$rootScope,$uibModal,$sce,$filter){
    ...
    $scope.search = function(item){
        ...
        if ( ($filter('lowercase')(item.file_name).indexOf($filter('lowercase')($scope.searchkey)) != -1) ||($filter('lowercase')(item.name).indexOf($filter('lowercase')($scope.searchkey)) != -1) || ($filter('lowercase')(item.description).indexOf($filter('lowercase')($scope.searchkey)) != -1)  ){
            return true;
        }
...
jungledrone.controller('documenttrashlist',function($scope,$state,$http,$cookieStore,$rootScope,$uibModal,$sce,$filter){
    $scope.trustAsHtml=$sce.trustAsHtml;
    ...
    $http({
        method:'POST',
        async:false,
        url:$scope.adminUrl+'document/trashlist',
        headers :   { 'Content-Type': 'application/x-www-form-urlencoded' }
    }).success(function(data){
        $scope.documentlist=data;
    ...
    $scope.docdelperm = function(item,size){

        $scope.currentindex=$scope.documentlist.indexOf(item);

        $uibModal.open({
            animation: true,
            templateUrl: 'docdelperm.html',
            controller: 'ModalInstanceCtrl',
            size: size,
            scope:$scope
        });
    ...
    $scope.docresotre = function(item,size){

        $scope.currentindex=$scope.documentlist.indexOf(item);

        $uibModal.open({
            animation: true,
            templateUrl: 'docresotre.html',
            controller: 'ModalInstanceCtrl',
            size: size,
            scope:$scope
        });
...
jungledrone.controller('editprofile',function($scope,$state,$http,$cookieStore,$rootScope,Upload,$sce,$uibModal,$stateParams) {
    ...
    $scope.updateinfo = function () {
        ...
        $http({
            ...
            $cookieStore.put('userfullname', data.fname + ' ' + data.lname);
...
jungledrone.controller('mydownloads',function($scope,$state,$http,$cookieStore,$rootScope,Upload,$sce,$uibModal,$stateParams,$filter){
    ...
    $http({
        ...
        //$scope.documentlist=data;
        $scope.documentlist=[];
       // alert($cookieStore.get('userroles'));
        console.log($cookieStore.get('userroles'));
        var x;
        for(x in data){
            //var i=getMatch(data[x].share_with_array,  $cookieStore.get('userroles'));
            //alert(data[x].share_with_array);
            //alert(i);
            var y;
            for(y in data[x].share_with_array){
                //if(data[x].share_with_array.id)
                if($.inArray(data[x].share_with_array[y].id, $cookieStore.get('userroles') )>-1){
                    $scope.documentlist.push(data[x]);
                }
            }
    ...
    $scope.search = function(item){
        ...
        if ( ($filter('lowercase')(item.file_name).indexOf($filter('lowercase')($scope.searchkey)) != -1) || ($filter('lowercase')(item.description).indexOf($filter('lowercase')($scope.searchkey)) != -1)  ){
            return true;
        }
...
jungledrone.controller('rolelist', function($scope,$state,$http,$cookieStore,$rootScope) {
    ...
    $scope.delrole = function(item){
        ...
        $http({
            ...
            data    : $.param({role_id: $scope.rolelist[idx].id}),  // pass in data as strings
            headers : { 'Content-Type': 'application/x-www-form-urlencoded' }
        }) .success(function(data) {
            $rootScope.stateIsLoading = false;
...
jungledrone.controller('addlegend',function($scope,$state,$http,$cookieStore,$rootScope,Upload,$sce,$uibModal){
    ...
    $scope.dosubmit=function() {
            ...
            $http({
                method  :   'POST',
                async   :   false,
                url :       $scope.adminUrl+'legends/add',
                data    : $.param($scope.form),
                headers :   { 'Content-Type': 'application/x-www-form-urlencoded' }
...
jungledrone.controller('legendlist',function($scope,$state,$http,$cookieStore,$rootScope,$uibModal,$sce,$filter){
    ...
    $http({
        method:'POST',
        async:false,
        url:$scope.adminUrl+'legends/list',
        headers :   { 'Content-Type': 'application/x-www-form-urlencoded' }
    }).success(function(data){
        $scope.legendlist=data;
    })
    ...
    $scope.search = function(item){
        ...
        if ( ($filter('lowercase')(item.abreviation).indexOf($filter('lowercase')($scope.searchkey)) != -1) ||($filter('lowercase')(item.title).indexOf($filter('lowercase')($scope.searchkey)) != -1) ){
            return true;
        }
    ...
    $scope.dellegend = function(item,size){

        $scope.currentindex=$scope.legendlist.indexOf(item);

        $uibModal.open({
            animation: true,
            templateUrl: 'legenddelconfirm.html',
            controller: 'ModalInstanceCtrl',
            size: size,
            scope:$scope
        });
    ...
    $scope.changelegendstatus = function(item,size){

        $scope.currentindex=$scope.legendlist.indexOf(item);

        $uibModal.open({
            animation: true,
            templateUrl: 'legendstatusfirm.html',
            controller: 'ModalInstanceCtrl',
            size: size,
            scope:$scope
        });
...
jungledrone.controller('editlegend',function($scope,$state,$http,$cookieStore,$rootScope,Upload,$sce,$uibModal,$stateParams){
    $scope.id=$stateParams.id;

    $http({
        method  : 'POST',
        async:   false,
        url     :     $scope.adminUrl+'legends/details',
        data    : $.param({'id':$scope.id}),  // pass in data as strings
        headers : { 'Content-Type': 'application/x-www-form-urlencoded' }
    }) .success(function(data) {
    ...
    $scope.dosubmit=function() {
            $http({
                method  :   'POST',
                async   :   false,
                url :       $scope.adminUrl+'legends/update',
                data    : $.param($scope.form),
                headers :   { 'Content-Type': 'application/x-www-form-urlencoded' }
